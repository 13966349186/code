 ...
 * 发送邮件示例
		...
		$this->smtp->host = IOSS_Conf::getConfig('mail_host');
		$this->smtp->port = IOSS_Conf::getConfig('mail_port');
		$this->smtp->user = IOSS_Conf::getConfig('mail_user');
		$this->smtp->pass = IOSS_Conf::getConfig('mail_pwd');
		if($this->smtp->sendmail('接收人', '邮件标题', '邮件内容')){
			die("发送成功");
		}else{
			die("发送失败");
		}
...
class qb_smtp
{
    ...
    function connect($params = array())
    {
        ...
        else
        {
            ...
            if(!socket_set_timeout($this->connection, 10, 250000)){
            	return FALSE;
            }
...
class smtp
{
	...
	function sendmail($to,/*$from, */$subject, $content, $T='HTML'){
//		$this->host = IOSS_Conf::getConfig('mail_host');
//		$this->port = IOSS_Conf::getConfig('mail_port');
//		$this->user = IOSS_Conf::getConfig('mail_user');
//		$this->pass = IOSS_Conf::getConfig('mail_pwd');
		if(empty($this->host) || empty($this->port) || empty($this->user) || empty($this->pass)){
			$this->error = "无法发送，请管理员先设置邮件服务器！";
			return false;
		}
		$from = $this->user;
		$CI = &get_instance();
		...
		$headers[] = "To:=?UTF-8?B?".base64_encode($this->name)."?= <$email[0]>";
		$headers[] = "From:=?UTF-8?B?".base64_encode($this->name)."?= <".$_CFG['smtp_mail'].">";
