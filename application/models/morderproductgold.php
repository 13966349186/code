 ...
 * @author lifw
 */
class MOrderProductGold extends MY_Model {
	protected $table = 'order_product_gold';
	...
	public function getDtlById($order_product_id){
		$this->db->select($this->table.'.*, order_product.*');
		$this->db->from($this->table);
		$this->db->join('order_product', $this->table.'.order_product_id=order_product.id', 'left');
    ...
    public function addCart($item){
    	...
    	$myItem = (Object)Array('order_product_id'=>$order_product->id, 'gold_num'=>$item->gold_num);
    	if($this->db->_trans_status){
